...
class SensorMonitor():
    ...
    def run_schedule(self):
        while True:
            # Get CPU Temp (C)
            self.pi_cpu_temp = check_output(["/opt/vc/bin/vcgencmd", "measure_temp"]).decode("utf-8").replace("temp=", "")

            # Get system uptime
            self.uptime = check_output(["/usr/bin/uptime", "-p"]).decode("utf-8").lstrip().replace("up ", "").replace(", ", "\n")
